...
float signed_volume(const Point &a, const Point &b, const Point &c, const Point &d) {
    ...
    part2 = b - d;
    part3 = c - d;
    part4 = part2.cross(part3);
    return ((part1.dot(part4))/6);
}
...
Rows face_rows(const Point& vf0, const Point& vf1, const Point& vf2, float voxel_size) {
    ...
    std::vector<Point> fbbox;
    fbbox.emplace_back(fmin_x, fmin_y, fmin_z);
    fbbox.emplace_back(fmin_x + fno_x * voxel_size, fmin_y + fno_y * voxel_size, fmin_z + fno_z * voxel_size);
